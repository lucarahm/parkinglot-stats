...
def interpolate_data(time_array, taken_array):
    if not time_array:
        # If there is no data, return default values for the hours 8 to 18
        default_hours = np.arange(8, 19)
        default_taken_array = np.zeros_like(default_hours)
        default_time_array = [Datetime.time(int(hour), int((hour % 1) * 60)) for hour in default_hours]
        return default_time_array, default_taken_array
...
def lambda_handler(event, context):
    ...
    weekday_str = str(weekday) + '-' + weekdays[weekday]
    ...
    plt.title(f'Average Taken Spots for {weekdays[weekday]}')
    plt.xlabel('Time')
